...
1.  **데이터 크롤링**:
    ...
    *   **폴더 분산 저장**: 파일 개수가 많아짐에 따라 1000회차 단위로 폴더를 나누어 저장합니다. (예: `1-1000/`, `1001-2000/`)
...
4.  **자동 업데이트**:
    *   GitHub Actions를 통해 매주 토요일(추첨 직후) 자동으로 스크립트가 실행되어 저장소를 최신 상태로 유지합니다.

## 📂 데이터 구조

모든 데이터는 `lotto_data/` 디렉토리에 저장됩니다. 파일 관리를 위해 1000회차 단위로 하위 폴더에 분산 저장됩니다.
...
*   실행이 완료되면 `lotto_data/` 폴더 내의 파일들과 `frequency.lotto`가 갱신됩니다.

## 🤖 GitHub Actions 자동화

이 저장소는 `.github/workflows/update_lotto.yml` 설정에 의해 자동화되어 있습니다.

*   **일정**: 매주 토요일 22:00 KST (토요일 13:00 UTC)
*   **동작**:
    1.  환경 설정 및 의존성 설치
